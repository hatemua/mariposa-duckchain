...
import { SimpleAgent, Utils } from '../src/SimpleAgent';

async function runSimpleExample(privateKey: string, address: string) {
...
function setupEventListeners(agent: SimpleAgent) {
...
function demonstrateUtilities() {
  ...
  const amount = Utils.toBigNumber('123.456789');
  console.log(`  Original: ${amount.toString()}`);
  console.log(`  Formatted (2 decimals): ${Utils.formatAmount(amount, 2)}`);
  
  console.log('\n🔍 Address Validation:');
  const validAddress = '0x1234567890abcdef1234567890abcdef12345678';
  const invalidAddress = 'invalid-address';
  console.log(`  Valid address (${validAddress.substring(0, 10)}...): ${Utils.isValidAddress(validAddress)}`);
  console.log(`  Invalid address: ${Utils.isValidAddress(invalidAddress)}`);
  
  console.log('\n📉 Slippage Calculation:');
  const originalAmount = '100';
  const slippageAmount = Utils.calculateSlippage(originalAmount, 5); // 5% slippage
  console.log(`  Original: ${originalAmount}`);
  console.log(`  With 5% slippage: ${slippageAmount}`);
}

// Example of different ways to run this
export function createSimpleAgent(privateKey: string, address: string): SimpleAgent {
  const config = {
    privateKey,
    address,
