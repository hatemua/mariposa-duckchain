...
class GeckoTerminalAPI {
    ...
    lastRequestTime = 0;
    MIN_REQUEST_INTERVAL = 1000; // 1 second between requests (optimized for responsiveness)
    requestCounter = 0;
    errorCounter = 0;
    logGeckoTerminalURL(url, context = '') {
        this.requestCounter++;
        const timestamp = new Date().toISOString();
        console.log(`🌐 [GECKO URL] [${timestamp}] [REQ#${this.requestCounter}] ${context ? `[${context}] ` : ''}${url}`);
    }
    logAPIResponse(context, success, statusCode, responseSize, duration) {
        const timestamp = new Date().toISOString();
    ...
    async fetchNetworks() {
        ...
        try {
            ...
            const networks = data.data.map((network) => ({
                id: network.id,
    ...
    async fetchDexesByNetwork(networkId, page = 1) {
        await this.enforceRateLimit();
        ...
        const data = await response.json();
        return data.data.map((dex) => ({
            id: dex.id,
            name: dex.attributes.name
        }));
    }
    async fetchPoolsByDex(networkId, dexId, page = 1) {
        await this.enforceRateLimit();
    ...
    async fetchAllPoolsByDex(networkId, dexId) {
        const allPools = [];
        let page = 1;
    ...
    async fetchNewPools(networkId, hoursBack = 24) {
    ...
    async fetchTrendingPools(networkId) {
        ...
        try {
            ...
            const trendingPools = allPools.filter(pool => {
                const liquidityUSD = parseFloat(pool.reserve_in_usd || '0');
                const volume24h = parseFloat(pool.volume_usd?.h24 || '0');
                const priceChange24h = Math.abs(parseFloat(pool.price_change_percentage?.h24 || '0'));
                // Trending pools have:
            ...
            trendingPools.sort((a, b) => {
                const volumeA = parseFloat(a.volume_usd?.h24 || '0');
                const volumeB = parseFloat(b.volume_usd?.h24 || '0');
                const changeA = Math.abs(parseFloat(a.price_change_percentage?.h24 || '0'));
                const changeB = Math.abs(parseFloat(b.price_change_percentage?.h24 || '0'));
    ...
    async fetchPoolsByNetwork(networkId, page = 1) {
        const startTime = Date.now();
    ...
    async fetchPoolData(networkId, poolAddress) {
        await this.enforceRateLimit();
    ...
    async fetchOHLCV(networkId, poolAddress, timeframe = 'hour', aggregate = '1') {
        await this.enforceRateLimit();
    ...
    async fetchTokenPrice(networkId, tokenAddresses) {
        await this.enforceRateLimit();
    ...
    async searchPools(query, networkId) {
        await this.enforceRateLimit();
...
class PoolAnalyzer {
    static analyzeNewToken(pool, tokenData) {
        const liquidityUSD = parseFloat(pool.reserve_in_usd || '0');
        const volume24h = parseFloat(pool.volume_usd?.h24 || '0');
    ...
    static analyzeTrendingPool(pool) {
        const liquidityUSD = parseFloat(pool.reserve_in_usd || '0');
        const volume24h = parseFloat(pool.volume_usd?.h24 || '0');
        const priceChange24h = Math.abs(parseFloat(pool.price_change_percentage?.h24 || '0'));
        const volumeToLiquidityRatio = liquidityUSD > 0 ? volume24h / liquidityUSD : 0;
...
class MarketDataProcessor {
    static formatPoolData(poolData) {
        ...
        if (attributes.price_change_percentage) {
            ...
            if (changes.h24)
                result += `24h Change: ${changes.h24}%\n`;
            if (changes.h1)
                result += `1h Change: ${changes.h1}%\n`;
        }
        return result;
    }
    static formatNetworksList(networks) {
    ...
    static formatPoolsList(pools) {
        let result = 'Pools:\n\n';
        pools.forEach(pool => {
            result += `${pool.name} (${pool.address})\n`;
            result += `  Reserve: $${pool.reserve_in_usd || 'N/A'}\n`;
            if (pool.volume_usd && pool.volume_usd.h24) {
                result += `  24h Volume: $${pool.volume_usd.h24}\n`;
            }
            result += '\n';
        });
        return result;
    }
    static formatOHLCV(ohlcvData) {
        ...
        result += '─'.repeat(80) + '\n';
        ohlcvList.slice(-10).forEach((ohlcv) => {
    ...
    static formatNewPools(pools) {
        ...
        pools.forEach((pool, index) => {
            ...
            const priceChange24h = pool.price_change_percentage?.h24;
            if (priceChange24h) {
            ...
            if (liquidityUSD < 10000)
                ...
                estimatedAge = '1-7 days';
            else if (liquidityUSD < 200000)
                estimatedAge = '1-4 weeks';
            else
                estimatedAge = '> 1 month';
            result += `   Estimated Age: ${estimatedAge}\n`;
            result += `   Network: ${pool.network}\n\n`;
        });
        return result;
    }
    static formatTrendingPools(trendingPools) {
    ...
    static formatNewTokens(newTokens) {
...
class MarketMCPServer {
    ...
    async getNewTokensFromPools(pools, count = 10) {
        ...
        for (let i = 0; i < processingLimit && newTokens.length < count * 2; i++) {
            ...
            try {
                ...
                const baseToken = poolData.included?.find((item) => item.type === 'token' && item.id === poolData.data.relationships.base_token.data.id);
                if (baseToken) {
                    const tokenData = {
                        id: baseToken.id,
    ...
    async handleGetNetworkPools(args) {
    ...
    async handleGetTokenPrices(args) {
    ...
    async handleGetTrendingPools(args) {
    ...
    calculateRiskScore(pool) {
        ...
        if (tokenType !== 'stablecoin') {
            const change24h = Math.abs(parseFloat(pool.price_change_percentage?.h24 || '0'));
            if (change24h > 50)
                risk += 20;
    ...
    calculateProfitScore(pool) {
        ...
        if (volume24h > 100000)
            ...
            profit += 10;
        // Price momentum
        const change24h = parseFloat(pool.price_change_percentage?.h24 || '0');
        if (change24h > 20)
            profit += 20;
        else if (change24h > 10)
            profit += 15;
        ...
        if (marketCap > 0 && marketCap < 1000000)
            profit += 15; // Small cap growth potential
        else if (marketCap < 10000000)
            profit += 10; // Mid cap potential
        else if (marketCap < 100000000)
            profit += 5; // Large cap stability
        // Transaction count indicates real usage
        const txCount = (pool.transactions?.h24?.buys || 0) + (pool.transactions?.h24?.sells || 0);
        if (txCount > 500)
            profit += 10;
        else if (txCount > 100)
            profit += 8;
    ...
    async handleGetAllTokens(args) {
        ...
        try {
            ...
            tokensWithScores.forEach((tokenData, index) => {
                ...
                output += `   24h Volume: $${parseFloat(pool.volume_usd?.h24 || '0').toLocaleString()}\n`;
                output += `   24h Change: ${pool.price_change_percentage?.h24 || '0'}%\n`;
                ...
                output += `   Transactions 24h: ${(pool.transactions?.h24?.buys || 0) + (pool.transactions?.h24?.sells || 0)}\n`;
